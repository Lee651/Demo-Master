...
public class FileController {
    ...
    public void newExport(HttpServletResponse response) throws IOException {
        ...
        try {
            ...
            WriteSheet writeSheet = EasyExcel.writerSheet(0, "用户" + 1).head(UserEntity.class).registerWriteHandler(new LongestMatchColumnWidthStyleStrategy()).build();
            excelWriter.write(exportList, writeSheet);
            ...
            for (int i = 1; i < (count / pageSize) + 1; i++) {
                ...
                writeSheet = EasyExcel.writerSheet(i, "用户" + (i + 1)).head(UserEntity.class).registerWriteHandler(new LongestMatchColumnWidthStyleStrategy()).build();
                excelWriter.write(exportList, writeSheet);
